...
const string outputFile = "nbody_seq.data";
...
void randomInitPlanets() {
    ...
    for (int i = 0; i < PLANET_NUM; i++) {
		...
		srand(time(0));
        p.mass = rand() % 100;//mass change point
        nbody.push_back(p);
        candidates.pop_back();
    }
}
...
void *refresh(void *data) {
	...
	QTree *r = inputData->r;
	int start = inputData->start;
	int end = inputData->end;
...
//    recording();
    ...
    for (int i = 0; i < SIM_DURATION; i++) {
        ...
        for (int j = 0; j < threadNum; j++) {
			data[j].r = r;
			data[j].start = (PLANET_NUM / threadNum) * j;
			data[j].end = (PLANET_NUM / threadNum) * (j + 1);
			rc = pthread_create(&thread[j], NULL, refresh, &data[j]);
		}
		for (int j = 0; j < threadNum; j++) {
